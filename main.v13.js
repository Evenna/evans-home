...
function initAvatar() {
  ...
  const coreVert = /* glsl */`
    ...
    void main() {
      ...
      // 多频动态形变：让球面像有机生物一样起伏
      // 用 position 采样，每个粒子相位不同 → 各自独立起伏
      float warp =
          sin(position.x * 2.8 + uTime * 0.31) * cos(position.z * 2.3 + uTime * 0.19) * 0.007
        + sin(position.y * 3.5 - uTime * 0.24) * cos(position.x * 2.1 + uTime * 0.28) * 0.005
        + sin(position.z * 5.2 + position.y * 3.8 + uTime * 0.17) * 0.004
        + sin(position.x * 8.1 + position.y * 6.3 + uTime * 0.53) * 0.003
        + sin(position.y * 7.4 - position.z * 6.2 + uTime * 0.41) * 0.003;
