...
function initAvatar() {
  ...
  for (let i = 0; i < N_CORE; i++) {
    ...
    coreSz[i]      = 0.013 + Math.random() * 0.010;
  }
...
async function runIntro() {
  ...
  await sleep(5200); // 4200ms流 + 800ms淡出过渡 + 200ms余量
...
function enterDatastream() {
  ...
  const CODE_FRAGS = [
    'SELECT * FROM users', 'WHERE user_id =', 'INNER JOIN sessions',
    'eval(input)', 'exec(cmd)', 'import sys',
    'void* ptr =', 'malloc(sizeof',  'free(ptr)',
    'for(i=0;i<n;i++)', 'while(true){', 'try{catch(e)',
    'AES.decrypt(', 'SHA256.hash(', 'JWT.verify(',
    '0x7fff4a2b', '0xdeadbeef', 'null ptr ref',
    'scene_id=S07', 'user_hash=e8f2', 'timestamp=',
    'CLASSIFIED', 'ACCESS:0x1', 'memory_dump',
  ];
  ...
  const colData = Array.from({ length: COLS }, (_, i) => ({
    ...
    frag: Math.random() < 0.12 ? CODE_FRAGS[Math.floor(Math.random() * CODE_FRAGS.length)] : null,
    fragTimer: 0,
  }));

  let startTime = null;
  const STREAM_DURATION = 4200; // ms 数据流持续时间
  ...
  function drawFrame(ts) {
    ...
    for (let i = 0; i < COLS; i++) {
      ...
      if (col.fragTimer <= 0 && Math.random() < 0.003) {
        ...
        col.fragTimer = 60 + Math.random() * 40;
      }

      if (col.frag && col.fragTimer > 0) {
        // 代码片段：绿白色，横向显示
        ctx.fillStyle = `rgba(160, 220, 180, ${col.brightness * 0.9})`;
        ctx.fillText(col.frag[Math.floor(col.fragTimer) % col.frag.length], x - 4, y);
      } else {
        // 头部字符：亮白
        ctx.fillStyle = `rgba(220, 240, 230, ${col.brightness})`;
        ctx.fillText(CHARS[Math.floor(Math.random() * CHARS.length)], x, y);

        // 下方字符：渐暗绿
        for (let j = 1; j < 20; j++) {
          const alpha = col.brightness * (1 - j / 20) * 0.6;
          ctx.fillStyle = `rgba(80, 180, 120, ${alpha})`;
          ctx.fillText(CHARS[Math.floor(Math.random() * CHARS.length)], x, y - j * 14);
        }
      }
