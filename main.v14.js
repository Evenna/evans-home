...
function initAvatar() {
  ...
  for (let i = 0; i < N_CORE; i++) {
    ...
    // 多频噪声让球面凹凸不规则：低频大幅形变 + 高频细节
    const noise =
      Math.sin(nx * 3.1 + ny * 2.7) * Math.cos(nz * 2.4) * 0.07
    + Math.sin(nx * 6.5 - nz * 5.3) * Math.cos(ny * 4.8) * 0.04
    + Math.sin(ny * 9.2 + nz * 7.1) * 0.02
    + (Math.random() - 0.5) * 0.04;

    const r = CORE_R * (1.0 + noise);
    corePos[i*3+0] = nx * r;
    corePos[i*3+1] = ny * r;
    corePos[i*3+2] = nz * r;
    coreNorm[i*3+0] = nx;  // 保存原始法线方向
    coreNorm[i*3+1] = ny;
    coreNorm[i*3+2] = nz;
  ...
  const coreVert = /* glsl */`
    ...
    void main() {
      ...
      // 用 aNorm（原始法线，世界空间固定）采样 warp → 起伏在世界中流动，不随旋转粘在粒子上
      float warp =
          sin(aNorm.x * 2.8 + uTime * 0.31) * cos(aNorm.z * 2.3 + uTime * 0.19) * 0.007
        + sin(aNorm.y * 3.5 - uTime * 0.24) * cos(aNorm.x * 2.1 + uTime * 0.28) * 0.005
        + sin(aNorm.z * 5.2 + aNorm.y * 3.8 + uTime * 0.17) * 0.004
        + sin(aNorm.x * 8.1 + aNorm.y * 6.3 + uTime * 0.53) * 0.003
        + sin(aNorm.y * 7.4 - aNorm.z * 6.2 + uTime * 0.41) * 0.003;
