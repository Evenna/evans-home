...
function initAvatar() {
  ...
  for (let i = 0; i < N_CORE; i++) {
    ...
    // pow(0.45) → 两极密集；再加中心空洞：拒绝太靠近赤道的粒子（用rejection sampling近似）
    ...
    // 中心疏：半径加上空洞偏移，离球心越近的区域粒子越少
    // 用 |cosP|（纬度）控制半径扰动，赤道附近r缩小一点让视觉上中间疏
    const latFactor = Math.abs(cosP); // 0=赤道 1=极点
    // 赤道粒子概率50%跳过 → 中心区域稀疏
    if (latFactor < 0.25 && Math.random() < 0.55) { i--; continue; }
...
function initHUD() {
  ...
  setInterval(() => {
    ...
    if (el) el.textContent = (97.8 + Math.random() * 1.4).toFixed(1) + '%';
  }, 3200);
}
...
function enterDatastream() {
  ...
  ['hud-corners','hud-header','hud-footer','hud-left','hud-right'].forEach(id => {
