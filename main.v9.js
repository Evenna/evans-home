...
function initAvatar() {
  ...
  for (let i = 0; i < N_CORE; i++) {
    ...
    const noise =
      Math.sin(nx * 3.1 + ny * 2.7) * Math.cos(nz * 2.4) * 0.18
    + Math.sin(nx * 6.5 - nz * 5.3) * Math.cos(ny * 4.8) * 0.09
    + Math.sin(ny * 9.2 + nz * 7.1) * 0.04
    + (Math.random() - 0.5) * 0.12; // 随机毛刺
  ...
  const coreVert = /* glsl */`
    ...
    void main() {
      ...
      // 多频动态形变：让球面像有机生物一样起伏
      float warp =
          sin(dir.x * 2.8 + uTime * 0.31) * cos(dir.z * 2.3 + uTime * 0.19) * 0.12
        + sin(dir.y * 3.5 - uTime * 0.24) * cos(dir.x * 2.1 + uTime * 0.28) * 0.07
        + sin(dir.z * 5.2 + dir.y * 3.8 + uTime * 0.17) * 0.04;
