...
function initAvatar() {
  ...
  // 每颗粒子独立震动需要的随机参数
  const coreFreq  = new Float32Array(N_CORE); // 个人震动频率
  const coreAmp   = new Float32Array(N_CORE); // 个人震动幅度
  const coreDirX  = new Float32Array(N_CORE); // 震动方向 x
  const coreDirY  = new Float32Array(N_CORE); // 震动方向 y
  const coreDirZ  = new Float32Array(N_CORE); // 震动方向 z
  ...
  for (let i = 0; i < N_CORE; i++) {
    ...
    // 粒子分布在球体积内（不贴壳）：内层到外层，外层粒子更多
    // 用 r^(1/3) 的逆：volumetric sampling = cbrt(random) * CORE_R
    // 但我们要外密内疏（壳感保留）：r = CORE_R * (0.55 + 0.45 * random^0.4)
    // 再叠加方向性噪声让外轮廓不规则
    const radial = 0.60 + 0.40 * Math.pow(Math.random(), 0.45);
    const bumps  =
      Math.sin(nx * 3.7 + ny * 2.9) * Math.cos(nz * 3.1) * 0.22
    + Math.sin(nx * 7.3 - nz * 5.8 + ny * 4.2) * 0.10
    + Math.sin(ny * 11.1 + nz * 8.3) * 0.05;
    const r = CORE_R * (radial + bumps);
    ...
    coreSz[i]      = 0.012 + Math.random() * 0.011;

    // 每颗独立震动：方向随机，频率0.8~2.8，幅度0.03~0.18
    coreFreq[i]  = 0.8 + Math.random() * 2.0;
    coreAmp[i]   = 0.03 + Math.random() * 0.15;
    // 随机震动方向（归一化）
  ...
  coreGeo.setAttribute('aFreq',    new THREE.BufferAttribute(coreFreq,  1));
  coreGeo.setAttribute('aAmp',     new THREE.BufferAttribute(coreAmp,   1));
  coreGeo.setAttribute('aDirX',    new THREE.BufferAttribute(coreDirX,  1));
  coreGeo.setAttribute('aDirY',    new THREE.BufferAttribute(coreDirY,  1));
  coreGeo.setAttribute('aDirZ',    new THREE.BufferAttribute(coreDirZ,  1));
  ...
  const coreVert = /* glsl */`
    ...
    attribute float aDirX;
    attribute float aDirY;
    ...
    void main() {
      ...
      // 每颗粒子独立随机方向震动，产生有机不规则抖动
      float vibrate = sin(uTime * aFreq + aPhase * 6.28) * aAmp;
      vec3 jiggle   = vec3(aDirX, aDirY, aDirZ) * vibrate;

      // 全局呼吸：慢速整体膨胀收缩
      float breathe = 1.0 + sin(uTime * 0.18) * 0.04 + sin(uTime * 0.41) * 0.02;

      float pulse  = uSpeak * 0.15 * sin(uTime * 1.8 + aPhase * 5.0);
      float seed   = fract(sin(aPhase * 127.1) * 43758.5);
      float glitch = uGlitch * (seed - 0.5) * 0.50;

      vec3 p = position * breathe + jiggle + dir * (pulse + glitch);
      ...
      // 透明度随震动幅度变化：震动大的粒子更亮（能量感）
      float energy  = 0.5 + 0.5 * abs(sin(uTime * aFreq + aPhase * 6.28));
      float twinkle = 0.85 + 0.15 * sin(uTime * 0.6 + aPhase * 11.0);
      vA = (0.30 + aLat * 0.50 + energy * aAmp * 1.5) * twinkle * (1.0 + uSpeak * 0.35);
    }
  `;
  ...
  halGeo.setAttribute('position', new THREE.BufferAttribute(halPos,   3));
  halGeo.setAttribute('aPhase',   new THREE.BufferAttribute(halPhase, 1));
  halGeo.setAttribute('aSize',    new THREE.BufferAttribute(halSz,    1));
  ...
  const halVert = /* glsl */`
    ...
    varying float vA;
    void main() {
      float breathe = 1.0 + sin(uTime * 0.20 + aPhase) * 0.04;
  ...
  const halUni = { uTime:{value:0}, uSK:{value:SK}, uSpeak:{value:0}, uTex:{value:halTex} };
  const halMat = new THREE.ShaderMaterial({
    uniforms: halUni, vertexShader: halVert, fragmentShader: halFrag,
    blending: THREE.AdditiveBlending, depthWrite: false, transparent: true,
  });
  const halPts = new THREE.Points(halGeo, halMat);
